...
- **Global scope**: `C:/@code/spcs-transcripts/.cursor/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/@code/spcs-transcripts/.cursor/notes/`)
...
2. Glob `C:/@code/spcs-transcripts/.cursor/notes/*.md` (if directory exists) — global notes
...
Global (C:/@code/spcs-transcripts/.cursor/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/@code/spcs-transcripts/.cursor/notes/` — works in any directory
