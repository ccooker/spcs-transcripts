...
This plan lists 15 `files_modified`, but 8 of those files are generated by CLI commands (`npm create vite` and `npx shadcn init`) — the executor runs commands and does not write content for those files. Only 7 files require hand-written content by the executor:

| Type | Files |
|------|-------|
| CLI-generated (run command, not write) | `client/package.json`, `client/tsconfig.json`, `client/tsconfig.node.json`, `client/vite.config.ts` (template), `client/index.html`, `client/components.json`, `client/src/main.tsx` (overwritten after scaffold), `client/src/index.css` (extended after scaffold) |
| Hand-written by executor | `client/.env.example`, `client/src/auth/msalConfig.ts`, `client/src/pages/LoginPage.tsx`, `client/src/pages/HomePage.tsx`, `client/src/pages/UnauthorizedPage.tsx`, `client/src/components/ProtectedRoute.tsx`, `client/src/App.tsx` |

The effective executor context load is equivalent to a ~7-file plan. The high `files_modified` count reflects post-scaffold modifications to CLI-generated files, not wholesale authoring.
...
<task type="auto">
  ...
  <verify>
    <automated>cd client && npx tsc --noEmit && node -e "process.exit(require('fs').existsSync('components.json') ? 0 : 1)" && echo "client scaffold valid"</automated>
  </verify>
  <acceptance_criteria>
